...
import { listFile } from "../redux/slices/Directives";
...
export default function Directives() {
  ...
  const list = useSelector((_state) => _state.dataDirectives);
  ...
  const getList = (pageNo = 1) => {
    ...
    dispatch(
      listFile(token, "directives", pageNo, (error, message) => {
        if (error) {
      ...
      <View style={{ marginBottom: 40 }}>
        {!list.isLoading ? (
          list && list.filesList && list.filesList.length ? (
            ...
            <>
              <Text style={{ marginTop: 20 }}>No Directives Found</Text>
