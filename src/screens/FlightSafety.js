...
export default function FlightSafety() {
  ...
  const list = useSelector((_state) => _state.dataFlightSafety);
  ...
  const getList = (pageNo = 1) => {
    ...
    dispatch(
      listFile(token,'flight-safety', pageNo, (error, message) => {
        if (error) {
    ...
    const uploadDocument = async () => {
      try {
        ...
        if (singleFile != null) {
          ...
          data.append("type", "flight-safety");
      ...
      <View style={{marginBottom: 40}}>
        {!list.isLoading ? (
          list && list.filesList && list.filesList.length ? (
            ...
            <>
            <Text style={{ marginTop: 20 }}>No Forms Found</Text>
