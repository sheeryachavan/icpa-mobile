...
import { listFile } from "../redux/slices/Miscellaneous";
...
export default function Miscellaneous() {
  ...
  const list = useSelector((_state) => _state.dataMiscellaneous);
  ...
  const getList = (pageNo = 1) => {
    ...
    dispatch(
      listFile(token, "miscellaneous", pageNo, (error, message) => {
        if (error) {
  ...
  const uploadDocument = async () => {
    try {
      ...
      if (singleFile != null) {
        ...
        data.append("type", "miscellaneous");
      ...
      <View style={{ marginBottom: 40 }}>
        {!list.isLoading ? (
          list && list.filesList && list.filesList.length ? (
            ...
            <>
              <Text style={{ marginTop: 20 }}>No miscellaneous docs Found</Text>
