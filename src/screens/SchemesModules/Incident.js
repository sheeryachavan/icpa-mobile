...
import {phoneRegex} from '../../_const/const';
...
export function Incident({navigation}) {
  ...
  const handleSubmit = () => {
    ...
    if (firstName === '') {
      ...
      showTopErrorMessage('Incident Type is required', 'danger');
    } else if (description === '') {
      ...
      dispatch(
        raiseScheme(obj, token,2, (error, message) => {
          if (error) {
